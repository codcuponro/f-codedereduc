...
const Title = (props) => {
  const { title, buttonLabel, buttonHref, white, hideButton, hideHeading, h1 } = props
  return (
    <div className='flex flex-col md:flex-row md:justify-between items-start md:items-center gap-4 md:gap-10'>
      {h1 ? (
        <h1 className={`text-2xl md:text-[34px] md:leading-[40px] font-semibold text-dark ${hideHeading && 'hidden md:block'} ${white && 'text-pure'}`}>
          {title}
        </h1>
      ) : (
        <h2 className={`text-2xl md:text-[34px] md:leading-[40px] font-semibold text-dark ${hideHeading && 'hidden md:block'} ${white && 'text-pure'}`}>
          {title}
        </h2>
      )}
      {
